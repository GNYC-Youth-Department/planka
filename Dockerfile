...
# (Removed global npm update to prevent Alpine crashes)
RUN npm install \
  && npm run build \
...
# Copy the client source code (contains your GNYC branding)
COPY client .
...
# These are essential for the app to start and the healthcheck to run
